...
export default function NewLedgerPage({ params }: { params: Promise<{ storeId: string }> }) {
  ...
  const day_savings  = Number(form.day_savings) || 0
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error: upsertError } = await supabase
      ...
      .upsert({
        ...
        pay_out:       result.data.pay_out,
        bills:         result.data.bills,
        payroll:       result.data.payroll,
        day_savings:   result.data.day_savings,
        total_savings: result.data.total_savings,
  ...
  return (
    <div className="max-w-2xl">
      <PageHeader title="Add ledger entry" description="Record daily sales, payouts, bills, payroll and savings" />
      ...
      <Card className="p-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Input label="Sale ($)" type="number" step="0.01" min="0" value={form.sale}
              onChange={e => set('sale', e.target.value)} placeholder="0.00" />
            <Input label="Pay Out ($)" type="number" step="0.01" min="0" value={form.pay_out}
              onChange={e => set('pay_out', e.target.value)} placeholder="0.00" />
            <Input label="Bills ($)" type="number" step="0.01" min="0" value={form.bills}
              onChange={e => set('bills', e.target.value)} placeholder="0.00" />
            <Input label="Payroll ($)" type="number" step="0.01" min="0" value={form.payroll}
              onChange={e => set('payroll', e.target.value)} placeholder="0.00" />
            <Input label="Day Savings ($)" type="number" step="0.01" min="0" value={form.day_savings}
              onChange={e => set('day_savings', e.target.value)} placeholder="0.00" />
            <Input label="Total Savings ($)" type="number" step="0.01" min="0" value={form.total_savings}
              onChange={e => set('total_savings', e.target.value)} placeholder="0.00" />
          </div>

          <Textarea label="Notes (optional)" value={form.notes} onChange={e => set('notes', e.target.value)} placeholder="Any notes..." />
