...
export async function POST(request: NextRequest) {
  ...
  console.log(`[Payouts] Payout created successfully, triggering sync for date: ${result.data.date}`)
  try {
    const { syncCombinedLedger } = await import('@/lib/combined-sync')
    await syncCombinedLedger(supabase, result.data.date, user.id)
    console.log('[Payouts] Combined ledger sync completed')
  } catch (syncError) {
    console.error('[Payouts] Failed to sync combined ledger after payout:', syncError)
  }
